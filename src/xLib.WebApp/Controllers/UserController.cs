...
public class UserController : ApiControllerBase
{
    ...
    public async Task<IActionResult> GetUserByNameAsync(
        ...
        UpdateUserViewModel model, IFormFile? avatarFile,
        IFormFile? headerFile)
    {
        BlobResponseDto avatar = null;
        BlobResponseDto header = null;

        if (avatarFile != null) avatar = await Mediator.Send(new UploadAvatarAndHeaderProfileCommand(avatarFile));

        if (headerFile != null) header = await Mediator.Send(new UploadAvatarAndHeaderProfileCommand(headerFile));
